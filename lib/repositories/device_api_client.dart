...
class DeviceApiClient {
  ...
  Future<Device> fetchDevice(BluetoothDevice bluetoothDevice) async {
    ...
    for (BluetoothService service in services) {
      ...
      if (!listEquals(service.uuid.toByteArray(), [
        0x8d,
        0x53,
        0xdc,
        0x1d,
      ...
      for (BluetoothCharacteristic charac in characteristics) {
        ...
        if (!listEquals(charac.uuid.toByteArray(), [
          0xda,
          0x2e,
          0x78,
          0x28,
    ...
    if (smpCharac == null) {
      ...
      throw new Exception(
          'Device doesn\'t support Simple Management Protocol. You may need to flash a suitable firmware.');
    }
    ...
    smpCharac.value.listen((value) {
      ...
      if (response.length < 4) {
        return;
      } //  Length field not available
      final len =
          (response[2] << 8) + response[3]; //  Length field in bytes 2 and 3
